...
const IncomeExpenditureNote = ()=> {
    ...
    const { onDownload } = useDownloadExcel({
        ...
        filename        : 'income-expenditure-note',
        sheet           : 'Income Expenditure Note'
    });
    ...
    const [income_expenditure_note_list, setIncome_expenditure_note_list] = useState([]);
    ...
    const searchBalanceSheet = ()=> {
        ...
        if(company <= 0) {
            ...
            const axios = apiUrl.get("/accounts/income-expenditure-note/?company="+company+"&branch="+branch+"&closing_month="+closing_month);
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
                    ...
                    setIncome_expenditure_note_list(result_data.data.coa_data);
                    ...
                    setFinancial_year('')
                    setIncome_expenditure_note_list([]);
                    setSurplus_deficit('');
    ...
    let income_expenditure_note_note_cg = balance_sheet_note.balance_sheet_note_cg || 0;
    let income_expenditure_note_note_gl = balance_sheet_note.balance_sheet_note_gl || 0;

    return (
        <Layout>
            <HeaderTitle title={lang.income_expenditure_note} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.income_expenditure_note}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.income_expenditure_note}</span>
                            </div>
                        </div>
                ...
                <div className="row">
                    <div className="col-md-12 mg-b-15">
                                        ...
                                        <tbody>
                                            <tr className="">
                                                ...
                                                <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                    <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.income_expenditure_note}</span>
                                                </th>
                        ...
                        <div className="table-responsive">
                            ...
                            : <>
                            {income_expenditure_note_list.length > 0 ?
                            <>
                            <table className="table-striped table-hover table-centered align-middle table-nowrap" width="100%" ref={excelExportRef}>
                                ...
                                <tbody>
                                    {income_expenditure_note_list.map((cg_row) => (<>
                                        {cg_row.general_ledger.map((gl_row) => (<>
                                        <tr className='' key={gl_row.chart_of_accounts_id}>
                                            <th className="tx-left tx-uppercase" colSpan={12}>{(++income_expenditure_note_note_gl).toString().padStart(2, '0')}. {gl_row.chart_of_accounts_code} - {gl_row.chart_of_accounts_name}</th>
                                        </tr>
...
export default  IncomeExpenditureNote;
