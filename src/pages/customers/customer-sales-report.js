...
const CustomerSalesReport = ()=> {
    ...
    if (typeof window !== 'undefined') {
        ...
        // user_group =1 Super Admin, user_group =2 Admin, user_group =3 Manager, user_group =4 Accounts User, user_group =5 Purchase & Sales, user_group =6 Purchase, user_group =7 Sales
    ...
    const [customer_sales_report, setCustomer_sales_report]         = useState([]);
    const total_payable_amount  = customer_sales_report.reduce((amount, data) => amount + parseFloat((data.sales_payable_amount)), 0);
    const total_paid_amount     = customer_sales_report.reduce((amount, data) => amount + parseFloat((data.sales_paid_amount)), 0);
    const total_due_amount      = customer_sales_report.reduce((amount, data) => amount + parseFloat((data.sales_due_amount)), 0);
    ...
    const searchCustomerSalesReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/customers/customer-sales-report/?company="+company+"&branch="+branch+"&customer="+customer+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setCustomer_sales_report(result_data.data);
                setCompany_info(result_data.company);
                setBranch_info(result_data.branch);
                setCustomer_info(result_data.customer);
                setSearchButton(false);
            } else {
                setCustomer_sales_report([]);
                setCompany_info('');
                setBranch_info('');
                setCustomer_info('');
                setSearchButton(false);
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.customer_sales_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.customer_sales_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.customer_sales_report}</span>
                            </div>
                        </div>
                ...
                <div className="row">
                    <div className="col-md-12 mg-b-15">
                            ...
                            <div className="col-md-2 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchCustomerSalesReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.customer_sales_report}</span>
                                            </th>
                        ...
                        {searchButton?
                            ...
                            :<>
                            {customer_sales_report.length > 0 ?
                            <>
                            <table className="table table-striped table-bordered" width="100%">
                                ...
                                :
                                <tbody>
                                    {customer_sales_report.map((row, index) => {
                                    return (
