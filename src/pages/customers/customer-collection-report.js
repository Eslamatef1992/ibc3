...
const CustomerPaymentReport = ()=> {
    ...
    const [customer_collection_report, setCustomer_collection_report]         = useState([]);
    const total_paid_amount     = customer_collection_report.reduce((amount, data) => amount + parseFloat((data.customer_payment_paid)), 0);
    ...
    const searchCustomerPaymentReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/customers/customer-collection-report/?company="+company+"&branch="+branch+"&customer="+customer+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setCustomer_collection_report(result_data.data);
                setCompany_info(result_data.company);
                setBranch_info(result_data.branch);
                setCustomer_info(result_data.customer);
                setSearchButton(false);
            } else {
                setCustomer_collection_report([]);
                setCompany_info('');
                setBranch_info('');
                setCustomer_info('');
                setSearchButton(false);
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.customer_collection_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.customer_collection_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.customer_collection_report}</span>
                            </div>
                        </div>
                ...
                <div className="row">
                    <div className="col-md-12 mg-b-15">
                            ...
                            <div className="col-md-2 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchCustomerPaymentReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.customer_collection_report}</span>
                                            </th>
                        ...
                        {searchButton?
                            ...
                            :<>
                            {customer_collection_report.length > 0 ?
                            <>
                            <table className="table table-striped table-bordered" width="100%">
                                ...
                                :
                                <tbody>
                                    {customer_collection_report.map((row, index) => {
                                    return (
