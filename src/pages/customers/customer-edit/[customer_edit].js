...
const CustomerUpdate = ({data})=> {
    ...
    const [customer_picture_old, setCustomer_picture_old]             = useState('');
    ...
    const formSubmit = (e) => {
        ...
        formData.append('customer_picture_old', customer_picture_old);
        formData.append('customer_picture', customer_picture);
        formData.append('customer_status', customer_status);

        const axios = apiUrlFormData.put("/customers/customer-update/"+data,formData)
        axios.then((response) => {
    ...
    const customer_Data = () => {
        const axios = apiUrl.get("/customers/get-customer/"+data);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setCustomer_id(result_data.data.customer_id);
                setCompany(result_data.data.customer_company);
                setCustomer_name(result_data.data.customer_name);
                setCustomer_contact_person(result_data.data.customer_contact_person);
                setCustomer_phone_number(result_data.data.customer_phone_number);
                setCustomer_email(result_data.data.customer_email);
                setCustomer_address(result_data.data.customer_address);
                setCustomer_picture_old(result_data.data.customer_picture);
                setCustomer_status(result_data.data.customer_status);
            } else {
                setCustomer_id('');
                setCompany('');
                setCustomer_name('');
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.customer_edit} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.customer_edit}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.customer_edit}</span>
                            </div>
                        </div>
...
export const getServerSideProps = async (context) => {
    const data = context.params.customer_edit;
    return {
        props:{
            data
        }
