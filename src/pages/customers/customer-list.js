...
const CustomerList = ()=> {
    ...
    const formSubmitDelete = (delete_id) => {
        const axios = apiUrl.delete("/customers/customer-delete/"+delete_id,)
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                const deleteArray = customer_list.filter((data) => data.customer_id !== delete_id);
                setCustomer_list(deleteArray);
                setTimeout(() => {
    ...
    const customerData = () => {
        setOnProcess(true);
        const axios = apiUrl.get("/customers/customer-list/?company="+company+"&status="+status+"&search="+search);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.customer_list} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.customer_list}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.customer_list}</span>
                            </div>
                        </div>
                        <div className="d-flex align-items-center d-print-none">
                            ...
                            <Link className="btn btn-success rounded-pill pd-t-6-force pd-b-5-force mg-r-3" title={lang.new_customer} href="/customers/create"><i className="fal fa-plus"></i></Link>
                            : ''}
                            ...
                            <table className="table table-striped table-bordered" ref={excelExportRef}>
                                <thead className="tx-12 tx-uppercase">
                                    <tr>
                                        ...
                                        <th className="text-center">{lang.customer_name}</th>
                                ...
                                :
                                <tbody>
                                    {customer_list.map((row, index) => {
                                    return (
                                    <tr className='' key={row.customer_id}>
                                        <td className="tx-center">{(index+1).toString().padStart(2, '0')}</td>
                                        <td className="tx-center">
                                            <Image className="img wd-25 wd-sm-25 ht-25 ht-sm-25 rounded-circle" src={`https://${row.customer_picture}`} alt="Customer Picture" property="true" width={50} height={50} />
                                        </td>
                                        <td className="tx-left">{row.customer_name}</td>
                                        <td className="tx-center">{row.customer_phone_number}</td>
                                        <td className="tx-left">{row.customer_email}</td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.customer_sales} /></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.customer_paid} /></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.customer_due} /></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.customer_return} /></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.customer_return_paid} /></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.customer_return_due} /></td>

                                        <td className="tx-center">{row.customer_status==1?lang.active:lang.inactive}</td>
                                        <td className="tx-center d-print-none">
                                            ...
                                            <Link className="text-primary mg-r-3" href={`/customers/customer-edit/${row.customer_id}`} title={lang.edit}><i className="fas fa-pencil wd-16 mr-2"></i></Link>
                                            :''}
                                            {user_user_group == 1 ?
                                            <Link className="text-danger" href="#" title={lang.remove} onClick={() => formModalDelete(row.customer_id)}><i className="fas fa-times wd-16 mr-2"></i></Link>
                                            :''}
                                        </td>
                                    </tr>
                                    )})}
                ...
                <div className="modal-dialog modal-md">
                    <div className="modal-content">
                        <div className="modal-header bg-primary m-0 p-2">
                            <h6 className="modal-title text-white">{lang.delete} {lang.customer}</h6>
                        ...
                        <div className="modal-footer border-top p-2">
                            <button type="submit" className="btn btn-sm btn-success rounded-pill text-uppercase pd-t-6-force pd-b-5-force" onClick={() => formSubmitDelete(customer_id)}><i className="fal fa-check-circle"></i> {lang.yes}</button>
