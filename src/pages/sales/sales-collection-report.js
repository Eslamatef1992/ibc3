...
const SalesCollectionReport = ()=> {
    ...
    const payment_type_list = [
        {
            payment_type_id:1,
            payment_type_code: 'Sales',
            payment_type_name: 'On Sales'
        },
        {
            payment_type_id:2,
    ...
    const [sales_collection_report, setSales_collection_report] = useState([]);
    const total_payable_amount  = sales_collection_report.reduce((amount, data) => amount + parseFloat((data.customer_payment_payable)), 0);
    const total_paid_amount     = sales_collection_report.reduce((amount, data) => amount + parseFloat((data.customer_payment_paid)), 0);
    const total_due_amount      = sales_collection_report.reduce((amount, data) => amount + parseFloat((data.customer_payment_due)), 0);
    ...
    const searchSalesCollectionReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/sales/sales-collection-report/?company="+company+"&branch="+branch+"&customer="+customer+"&payment_type="+payment_type+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setSales_collection_report(result_data.data);
                setCompany_info(result_data.company);
                setBranch_info(result_data.branch);
                setCustomer_info(result_data.customer);
                setSearchButton(false);
            } else {
                setSales_collection_report([]);
                setCompany_info('');
                setBranch_info('');
                setCustomer_info('');
                setSearchButton(false);
    ...
    return (
        ...
        <Layout>
            ...
            <HeaderTitle title={lang.sales_collection_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.sales_collection_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.sales_collection_report}</span>
                            </div>
                        </div>
                ...
                <div className="row">
                    <div className="col-md-12 mg-b-15">
                            ...
                            <div className="col-md-3 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchSalesCollectionReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.sales_collection_report}</span>
                                            </th>
                        ...
                        {searchButton?
                            ...
                            :<>
                            {sales_collection_report.length > 0 ?
                            <>
                            <table className="table table-striped table-bordered" width="100%">
                                ...
                                :
                                <tbody>
                                    {sales_collection_report.map((row, index) => {
                                    return (
                                    <tr className='' key={row.customer_payment_id}>
                                        ...
                                        <td className="tx-left">{row.customer_payment_customer_name}</td>
                                        ...
                                        <td className="tx-center">{row.customer_payment_type}</td>
                                    </tr>
                                    )})}
                                    <tr className="">
...
export default SalesCollectionReport;