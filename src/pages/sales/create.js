...
const SalesCreate = ()=> {
    ...
    const [sales_total_purchase_amount, setSales_total_purchase_amount]       = useState(0);
    ...
    const totalEditAmount = (data) => {
        ...
        const paid_amount       = payable_amount>data.sales_paid_amount?data.sales_paid_amount:payable_amount;
        ...
        setSales_paid_amount(paid_amount);
    ...
    const formSubmit = (e) => {
        ...
        if(company <= 0) {
            ...
            const axios = apiUrl.post("/sales/sales-create/",{sales_data:sales_data, sales_details_data:sales_details_data})
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
                    ...
                    setSubmitButton(false);
                    setCustomer('');
                    setSales_date(new Date().toISOString().split('T')[0]);
                    setItem_list([]);
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.new_sales} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.new_sales}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.new_sales}</span>
                            </div>
                        </div>
