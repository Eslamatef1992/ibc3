...
const SalesReturnReport = ()=> {
    ...
    const [sales_return_report, setSales_return_report]         = useState([]);
    const total_sales_return_amount = sales_return_report.reduce((amount, data) => amount + parseFloat((data.sales_return_payable_amount)), 0);
    const total_paid_amount     = sales_return_report.reduce((amount, data) => amount + parseFloat((data.sales_return_paid_amount)), 0);
    const total_due_amount      = sales_return_report.reduce((amount, data) => amount + parseFloat((data.sales_return_due_amount)), 0);
    ...
    const searchSalesReturnReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/sales/sales-return-report/?company="+company+"&branch="+branch+"&customer="+customer+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setSales_return_report(result_data.data);
                setCompany_info(result_data.company);
                setBranch_info(result_data.branch);
                setCustomer_info(result_data.customer);
                setSearchButton(false);
            } else {
                setSales_return_report([]);
                setCompany_info('');
                setBranch_info('');
                setCustomer_info('');
                setSearchButton(false);
    ...
    return (
        ...
        <Layout>
            ...
            <HeaderTitle title={lang.sales_return_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.sales_return_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.sales_return_report}</span>
                            </div>
                        </div>
                    ...
                    <div className="col-md-12 mg-b-15">
                            ...
                            <div className="col-md-2 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchSalesReturnReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.sales_return_report}</span>
                                            </th>
                        ...
                        {searchButton?
                            ...
                            <table className="table table-striped table-bordered" width="100%">
                                ...
                                :
                                <tbody>
                                    {sales_return_report.map((row, index) => {
                                    return (
                                    <tr className='' key={row.sales_return_id}>
                                        ...
                                        <td className="tx-center">{row.sales_return_payment_status}</td>
                                    </tr>
                                    )})}
                                    <tr className="">
                                        <th className="tx-right text-uppercase" colSpan={4}>{lang.total}</th>
                                        <th className="tx-right"><AccountsNumberFormat amount={total_sales_return_amount} /></th>
