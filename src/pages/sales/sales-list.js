...
const SalesList = ()=> {
    ...
    const total_payable_amount  = sales_list.reduce((debit, data) => debit + parseFloat((data.sales_payable_amount)), 0);
    const total_paid_amount = sales_list.reduce((credit, data) => credit + parseFloat((data.sales_paid_amount)), 0);
    const total_due_amount      = sales_list.reduce((credit, data) => credit + parseFloat((data.sales_due_amount)), 0);
    ...
    const formSubmitDelete = (delete_id) => {
        ...
        axios.then((response) => {
            ...
            if(result_data.status == 1){
                const deleteArray = sales_list.filter((data) => data.sales_id !== delete_id);
                setSales_list(deleteArray);
                setTimeout(() => {
    ...
    const searchSales = () => {
        if(company <= 0) {
            ...
            const axios = apiUrl.get("/sales/sales-list/?company="+company+"&branch="+branch+"&from_date="+from_date+"&to_date="+to_date+"&status="+status+"&search="+search);
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.sales_list} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.sales_list}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.sales_list}</span>
                            </div>
                        </div>
                        <div className="d-flex align-items-center d-print-none">
                            ...
                            <Link className="btn btn-success rounded-pill pd-t-6-force pd-b-5-force mg-r-3" title={lang.new_sales} href="/sales/create"><i className="fal fa-plus"></i></Link>
                            : ''}
                            ...
                            <table className="table table-striped table-bordered" ref={excelExportRef}>
                                ...
                                <tbody>
                                    ...
                                    <tr className='' key={row.sales_id}>
                                        ...
                                        <td className="tx-center">{row.sales_status==1?lang.active:lang.inactive}</td>
                                        <td className="tx-center d-print-none">
                                            <Link className="text-info mg-r-3" href="#" onClick={()=> viewReceipt(row.sales_id)} title={lang.receipt}><i className="fas fa-file-invoice-dollar wd-16 mr-2"></i></Link>
                                            <Link className="text-warning mg-r-3" href="#" onClick={()=> viewInvoice(row.sales_id)} title={lang.invoice}><i className="fas fa-file-invoice wd-16 mr-2"></i></Link>
                                            {user_user_group == 1 || user_user_group == 2 || user_user_group == 3 || user_user_group == 4 || user_user_group == 5?
                                            <Link className="text-primary mg-r-3" href={`/sales/sales-edit/${row.sales_id}`} title={lang.edit}><i className="fas fa-pencil wd-16 mr-2"></i></Link>
                                            :''}
                                            {/* {user_user_group == 1 ?
                                            <Link className="text-danger" href="#" title={lang.remove} onClick={() => formModalDelete(row.sales_id)}><i className="fas fa-times wd-16 mr-2"></i></Link>
                                            :''} */}
                                        </td>
                                    </tr>
                                    )})}
                ...
                <div className="modal-dialog modal-md">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer border-top p-2">
                            <button type="submit" className="btn btn-sm btn-success rounded-pill text-uppercase pd-t-6-force pd-b-5-force" onClick={() => formSubmitDelete(sales_id)}><i className="fal fa-check-circle"></i> {lang.yes}</button>
