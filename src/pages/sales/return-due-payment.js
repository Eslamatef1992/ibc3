...
const ReturnDuePayment = ()=> {
    ...
    if (typeof window !== 'undefined') {
        ...
        // user_group =1 Super Admin, user_group =2 Admin, user_group =3 Manager, user_group =4 Accounts User, user_group =5 Sales & Sales_return, user_group =6 Sales, user_group =7 Sales_return
    ...
    const [sales_return_due_list, setSales_return_due_list] = useState([]);
    ...
    const total_payable_amount                      = sales_return_due_list.reduce((payable_amount, data) => payable_amount + parseFloat((data.payable_amount)), 0);
    const total_paid_amount                         = sales_return_due_list.reduce((paid_amount, data) => paid_amount + parseFloat((data.paid_amount)), 0);
    const total_due_amount                          = sales_return_due_list.reduce((due_amount, data) => due_amount + parseFloat((data.due_amount)), 0);
    ...
    const editItem = (row) => {
        ...
        const updatedArray = sales_return_due_list.map((data) =>
            data.sales_return_id === row.sales_return_id ? items : data
        );

        setSales_return_due_list(updatedArray);
    ...
    const formSubmit = (e) => {
        ...
        const due_list = sales_return_due_list.filter((data) => data.new_payment > 0);
        if(company <= 0) {
            ...
            const sales_return_data = due_list.map((row) => ({
                sales_return_id                         : row.sales_return_id,
                sales_return_paid_amount                : parseFloat(row.paid_amount)+parseFloat(row.new_payment),
                sales_return_due_amount                 : parseFloat(row.payable_amount)-(parseFloat(row.paid_amount)+parseFloat(row.new_payment)),
                sales_return_reference_number           : reference_number,
                sales_return_payment_type               : payment_type,
                sales_return_payment_method             : payment_method,
                sales_return_payment_status             : parseFloat(row.payable_amount)-(parseFloat(row.paid_amount)+parseFloat(row.new_payment)) > 0?'Due':'Paid',
            }));

            const customer_payment_return_data = due_list.map((row) => ({
                customer_payment_return_company            : company,
                customer_payment_return_branch             : branch,
                customer_payment_return_date               : payment_date,
                customer_payment_return_sales           : row.sales_return_sales,
                customer_payment_return_sales_invoice   : row.sales_return_sales_invoice,
                customer_payment_return_sales_return    : row.sales_return_id,
                customer_payment_return_customer           : customer,
                customer_payment_return_payable            : row.due_amount,
                customer_payment_return_paid               : row.new_payment,
                customer_payment_return_due                : parseFloat(row.due_amount)-parseFloat(row.new_payment),
                customer_payment_return_type               : 'Due',
                customer_payment_return_sales_reference_number  : reference_number,
                customer_payment_return_sales_payment_type      : payment_type,
                customer_payment_return_sales_payment_method    : payment_method,
                customer_payment_return_status             : payment_status
            }));

            const axios = apiUrl.post("/sales/return-due-payment-create/",{payment_data:payment_data, sales_return_data:sales_return_data, customer_payment_return_data:customer_payment_return_data})
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
    ...
    const viewInvoice = (customer, payment_date) => {
        window.open("/sales/return-due-payment-invoice/?company="+company+"&branch="+branch+"&customer="+customer+"&payment_date="+payment_date, "Popup", "width=700, height=700");
    };

    const viewReceipt = (data) => {
        window.open("/sales/return-due-payment-receipt/?company="+company+"&branch="+branch+"&customer="+customer+"&payment_date="+payment_date, "Popup", "width=700, height=700");
    };

    const searchSalesReturnDue = () => {
        if(company <= 0) {
            setMassage('Select Company');
            setWarningModal(true);
            ...
            const axios = apiUrl.get("/sales/sales-return-due-list/?company="+company+"&branch="+branch+"&customer="+customer);
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
                    const sales_return_data = result_data.data.map((row) => ({
                        sales_return_id         : row.sales_return_id,
                        sales_return_date       : row.sales_return_date,
                        sales_return_sales    : row.sales_return_sales,
                        sales_return_sales_invoice: row.sales_return_sales_invoice,
                        payable_amount      : row.sales_return_payable_amount,
                        paid_amount         : row.sales_return_paid_amount,
                        due_amount          : row.sales_return_due_amount,
                        new_payment         : 0
                    }));
                    setSales_return_due_list(sales_return_data || []);
                    setSearchButton(false);
                } else {
                    setSales_return_due_list([]);
                    setSearchButton(false);
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.return+' '+lang.due_payment} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.return+' '+lang.due_payment}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <Link className="breadcrumb-item" href="/sales_return">{lang.sales_return}</Link>
                                <span className="breadcrumb-item hidden-xs active">{lang.return+' '+lang.due_payment}</span>
                            </div>
                        </div>
                        <div className="d-flex align-items-center d-print-none">
                            <Link className="btn btn-success rounded-pill pd-t-6-force pd-b-5-force mg-r-3" title={lang.sales_return_list} href="/sales/sales_return-list"><i className="fal fa-bars"></i></Link>
                        </div>
                    </div>
                </div>
                {/* Breadcrumb End */}
                        ...
                        <div className="col-md-12 col-sm-12 mb-3">
                                ...
                                <div className="col-lg-2 col-md-2 mt-3">
                                    <div className="form-group">
                                        ...
                                        <div className="d-grid gap-2">
                                            <button type="submit" className={`btn btn-info pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchSalesReturnDue()}>{searchButton?lang.process: lang.search}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            {sales_return_due_list.length > 0?
                            <div className="table-responsive">
                                <table className="table table-striped table-bordered">
                                    ...
                                    :
                                    <tbody>
                                        {sales_return_due_list.map((row, index) => {
                                        return (
...
export default ReturnDuePayment;