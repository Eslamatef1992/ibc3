...
const VoucherList = () => {
	...
	const [voucher_type, setVoucher_type] = useState("all");
	const [status, setStatus] = useState("all");

	const [voucher_list, setVoucher_list] = useState([]);
	const total_debit = voucher_list.reduce(
		(debit, data) => debit + parseFloat(data.accounts_total_debit),
		0
	);
	const total_credit = voucher_list.reduce(
		(credit, data) => credit + parseFloat(data.accounts_total_credit),
		0
	);
		...
		axios
			.then((response) => {
				...
				if (result_data.status == 1) {
					const deleteArray = voucher_list.filter(
						(data) => data.accounts_id !== delete_id
					);
	...
	const branchData = () => {
		const axios = apiUrl.get("/branch-owner/branches/" + owner);
		axios
			.then((response) => {
				const result_data = response.data;
				if (result_data.status == 1) {
	...
	const searchVoucher = () => {
		...
		if (company <= 0) {
			...
			setMassage(lang.project_select_warning);
			setWarningModal(true);
			setSearchButton(false);
		} else {
			const axios = apiUrl.get(
				"/accounts/voucher-list/?company=" +
					company +
					"&branch=" +
					branch +
					"&from_date=" +
	...
	return (
		<Layout>
			<HeaderTitle title={lang.voucher_list} keywords="" description="" />
					...
					<div className="d-flex justify-content-between">
						<div className="clearfix">
							...
							<div className="breadcrumb pd-0 mg-0 d-print-none">
								...
								<Link className="breadcrumb-item" href="/voucher">
									{lang.voucher}
								</Link>
						...
						<div className="d-flex align-items-center d-print-none">
							...
							user_user_group == 4 ? (
								<Link
									...
									href="/voucher/new-voucher"
									title={lang.new_voucher}>
								...
								<div className="form-group">
									...
									<select
										...
										{branch_list.map((branch_row) => (
											<option
												key={branch_row.branch_id}
												value={branch_row.branch_id}>
							...
							<div className="col-md-2 mt-3">
								<div className="form-group">
									...
									<input
										...
										onChange={(e) => setVoucher_number(e.target.value)}
									/>
								</div>
							</div>
							<div className="col-md-2 mt-3">
								<div className="form-group">
									...
									<select
										...
										<option value="all">All</option>
										{voucher_type_list.map((voucher_type_row) => (
											<option
												key={voucher_type_row.voucher_type_id}
												value={voucher_type_row.voucher_type_id}>
												{voucher_type_row.voucher_type_name}
											</option>
										))}
									</select>
								</div>
							...
							<div className="col-md-3 mt-3 mb-5">
								<div className="form-group">
									...
									<div className="d-grid gap-2">
										<button
											type="submit"
											className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${
												searchButton ? "disabled" : ""
											}`}
											onClick={() => searchVoucher()}>
											{searchButton ? lang.process : lang.search}
										</button>
									</div>
								</div>
							</div>
							...
							<table
								...
								{searchButton ? (
									...
									<tbody>
										{voucher_list.map((row, index) => {
											return (
												<tr className="" key={row.accounts_id}>
													...
													<td className="tx-center">
														{new Date(row.accounts_posting_date).toLocaleString(
															"en-in",
															{
																day: "2-digit",
																month: "2-digit",
													...
													<td className="tx-center">
														{row.accounts_voucher_type_name}
													</td>
													<td className="tx-center">
														{row.accounts_voucher_number}
													</td>
													<td className="tx-left">{row.accounts_narration}</td>
													<td className="tx-right">
														<AccountsNumberFormat
															amount={row.accounts_total_debit}
														/>
													</td>
													<td className="tx-right">
														<AccountsNumberFormat
															amount={row.accounts_total_credit}
														/>
													</td>
													<td className="tx-center">
														{row.accounts_status == 1
															? lang.active
															: lang.inactive}
													</td>
													<td className="tx-center d-print-none">
														<Link
															className="text-warning mg-r-3"
															href="#"
															title={lang.project}
															onClick={() => viewVoucher(row.accounts_id)}>
															<i className="fas fa-eye wd-16 mr-2"></i>
														</Link>
														{user_user_group == 1 ||
														user_user_group == 2 ||
														user_user_group == 3 ? (
															<Link
																...
																href={`/voucher/voucher-edit/${row.accounts_id}`}
																title={lang.edit}
																target="_blank">
														...
														{user_user_group == 1 ? (
															<Link
																...
																title={lang.remove}
																onClick={() =>
																	formModalDelete(row.accounts_id)
																}>
										...
										<tr>
											<th className="tx-right tx-uppercase" colSpan={5}>
												{lang.total}
											</th>
					...
					<div className="modal-content">
						...
						<div className="modal-footer border-top p-2">
							<button
								...
								onClick={() => formSubmitDelete(voucher_id)}>
					...
					<div className="modal-content">
						<div className="modal-header bg-danger m-0 p-2">
							...
							<button
								type="button"
								className="btn-close"
								onClick={() => setWarningModal()}></button>
						</div>
			...
			{/* Voucher Modal Start*/}
			<div
				className={`modal fade zoomIn ${voucherModal ? "show d-block" : ""}`}>
				<div className="modal-dialog modal-lg">
					<div className="modal-content">
						<div className="modal-header bg-primary m-0 p-2">
							<h6 className="modal-title text-white">
								{lang.view} {lang.voucher}
							</h6>
							<button
								type="button"
								className="btn-close"
								onClick={() => voucherModalClose()}></button>
						</div>

						<div className="modal-body m-0 pl-3 pr-3 pt-0">
							<div className="row">
								<div className="col-md-12 mt-3">
									<h5 className="tx-center tx-bold text-decoration-underline tx-uppercase tx-14">
										{voucher_data.accounts_voucher_type_name}
									</h5>

									<table className="" width="100%" align="center">
										<tbody>
											<tr className="text-uppercase">
												<th className="tx-left" width="50%">
													{lang.voucher_no}:{" "}
													{voucher_data.accounts_voucher_number}
												</th>
												<th className="tx-right" width="50%">
													{lang.date}:{" "}
													{new Date(
														voucher_data.accounts_posting_date
									...
									<table className="" width="100%" align="center">
										<tbody>
											<tr className="">
												<th className="tx-left text-uppercase" width="10%">
													{lang.narration}:{" "}
												</th>
												<th className="tx-left" width="85%">
													{voucher_data.accounts_narration}
												</th>
											</tr>
										</tbody>
									</table>
									...
									{accounts_details.length > 0 ? (
										<tbody>
												...
												return (
													<tr className="" key={row.accounts_details_id}>
														...
														<td className="tx-left">
															{row.accounts_details_general_ledger_code} -{" "}
															{row.accounts_details_general_ledger_name}
														</td>
														<td className="tx-left">
															{row.accounts_details_subsidiary_ledger_code} -{" "}
															{row.accounts_details_subsidiary_ledger_name}
														</td>
														<td className="tx-right">
															<AccountsNumberFormat
											...
											<tr className="table-info text-uppercase">
												<th className="tx-right" colSpan={3}>
													{lang.total_amount}
												</th>
												<th className="tx-right">
													<AccountsNumberFormat
														amount={voucher_data.accounts_total_debit}
													/>
												</th>
												<th className="tx-right">
													<AccountsNumberFormat
														amount={voucher_data.accounts_total_credit}
													/>
												</th>
											</tr>
											<tr className="tx-uppercase">
												<th className="tx-left" colSpan={5}>
													...
													{numberToWords.toWords(
														voucher_data.accounts_total_debit
													)}{" "}
													Only.
												</th>
											</tr>
										...
										<tbody>
											<tr>
												<th
													className="tx-center tx-uppercase text-danger"
													colSpan="5">
													{lang.data_not_found}
												</th>
											</tr>
						...
						<div className="modal-footer border-top p-2">
							<button
								type="button"
								className="btn btn-sm btn-info rounded-pill text-uppercase pd-t-6-force pd-b-5-force"
								onClick={() => printVoucher(voucher_id)}>
								<i className="fal fa-print"></i> {lang.print}
							</button>
							<button
								type="button"
								className="btn btn-sm btn-danger rounded-pill text-uppercase pd-t-6-force pd-b-5-force"
								onClick={() => voucherModalClose()}>
								<i className="fal fa-times-circle"></i> {lang.close}
							</button>
						</div>
					</div>
