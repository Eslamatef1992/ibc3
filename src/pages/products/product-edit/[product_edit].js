...
const ProductEdit = ({data})=> {
    ...
    const [product_picture_old, setProduct_picture_old]             = useState('');
    ...
    const formSubmit = (e) => {
        ...
        const axios = apiUrlFormData.put("/product/product-update/"+product_id,formData)
        axios.then((response) => {
    ...
    const product_Data = () => {
        const axios = apiUrl.get("/product/get-product/"+data);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setProduct_id(result_data.data.product_id);
                setCompany(result_data.data.product_company);
                setProduct_code(result_data.data.product_code);
                setProduct_name(result_data.data.product_name);
                setProduct_model(result_data.data.product_model);
                setProduct_description(result_data.data.product_description);
                setProduct_category(result_data.data.product_category);
                setProduct_brand(result_data.data.product_brand);
                setProduct_type(result_data.data.product_type);
                setProduct_unit(result_data.data.product_unit);
                setProduct_unit_price(result_data.data.product_unit_price);
                setProduct_purchase_price(result_data.data.product_purchase_price);
                setProduct_sales_price(result_data.data.product_sales_price);
                setProduct_purchase_discount(result_data.data.product_purchase_discount);
                setProduct_sales_discount(result_data.data.product_sales_discount);
                setProduct_purchase_tax(result_data.data.product_purchase_tax);
                setProduct_sales_tax(result_data.data.product_sales_tax);
                setProduct_purchase_vat(result_data.data.product_purchase_vat);
                setProduct_sales_vat(result_data.data.product_sales_vat);
                setProduct_picture_old(result_data.data.product_picture);
                setProduct_status(result_data.data.product_status);
            } else {
                setProduct_id('');
                setCompany('');
                setProduct_code('');
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.product_edit} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.product_edit}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.product_edit}</span>
                            </div>
                        </div>
...
export const getServerSideProps = async (context) => {
    const data = context.params.product_edit;
    return {
        props:{
            data
        }
