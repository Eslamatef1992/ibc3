...
const PurchaseReport = ()=> {
    ...
    const [purchase_report, setPurchase_report]         = useState([]);
    const total_purchase_amount = purchase_report.reduce((amount, data) => amount + parseFloat((data.purchase_payable_amount)), 0);
    const total_paid_amount     = purchase_report.reduce((amount, data) => amount + parseFloat((data.purchase_paid_amount)), 0);
    const total_due_amount      = purchase_report.reduce((amount, data) => amount + parseFloat((data.purchase_due_amount)), 0);
    ...
    const searchPurchaseReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/purchase/purchase-report/?company="+company+"&branch="+branch+"&supplier="+supplier+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setPurchase_report(result_data.data);
    ...
    return (
        ...
        <Layout>
            <HeaderTitle title={lang.purchase_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.purchase_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.purchase_report}</span>
                            </div>
                        </div>
                            ...
                            <div className="col-md-2 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchPurchaseReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.purchase_report}</span>
                                            </th>
