...
const PurchaseReturnCollectionReport = ()=> {
    ...
    const [return_collection_report, setReturn_collection_report] = useState([]);
    const total_payable_amount  = return_collection_report.reduce((amount, data) => amount + parseFloat((data.supplier_payment_return_payable)), 0);
    const total_paid_amount     = return_collection_report.reduce((amount, data) => amount + parseFloat((data.supplier_payment_return_paid)), 0);
    const total_due_amount      = return_collection_report.reduce((amount, data) => amount + parseFloat((data.supplier_payment_return_due)), 0);
    ...
    const searchPurchaseReturnCollectionReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/purchase/return-collection-report/?company="+company+"&branch="+branch+"&supplier="+supplier+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setReturn_collection_report(result_data.data);
                setCompany_info(result_data.company);
                setBranch_info(result_data.branch);
                setSupplier_info(result_data.supplier);
                setSearchButton(false);
            } else {
                setReturn_collection_report([]);
                setCompany_info('');
                setBranch_info('');
                setSupplier_info('');
                setSearchButton(false);
    ...
    return (
        ...
        <Layout>
            ...
            <HeaderTitle title={lang.return_collection_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.return_collection_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.return_collection_report}</span>
                            </div>
                        </div>
                ...
                <div className="row">
                    <div className="col-md-12 mg-b-15">
                            ...
                            <div className="col-md-2 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchPurchaseReturnCollectionReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.return_collection_report}</span>
                                            </th>
                        ...
                        {searchButton?
                            ...
                            :<>
                            {return_collection_report.length > 0 ?
                            <>
                            <table className="table table-striped table-bordered" width="100%">
                                ...
                                :
                                <tbody>
                                    {return_collection_report.map((row, index) => {
                                    return (
                                    <tr className='' key={row.supplier_payment_return_id}>
                                        ...
                                        <td className="tx-left">{row.supplier_payment_return_supplier_name}</td>
...
export default PurchaseReturnCollectionReport;