...
const PurchaseReturnList = ()=> {
    ...
    const { onDownload } = useDownloadExcel({
        ...
        sheet           : 'Purchase Return List'
    });

    const [purchase_return_id, setPurchase_return_id]         = useState('');
    ...
    const total_payable_amount  = purchase_return_list.reduce((debit, data) => debit + parseFloat((data.purchase_return_payable_amount)), 0);
    const total_paid_amount = purchase_return_list.reduce((credit, data) => credit + parseFloat((data.purchase_return_paid_amount)), 0);
    const total_due_amount      = purchase_return_list.reduce((credit, data) => credit + parseFloat((data.purchase_return_due_amount)), 0);
    ...
    const formModalDelete = (primary_id) => {
        setPurchase_return_id(primary_id);
    ...
    const formSubmitDelete = (delete_id) => {
        ...
        axios.then((response) => {
            ...
            if(result_data.status == 1){
                const deleteArray = purchase_return_list.filter((data) => data.purchase_return_id !== delete_id);
                setPurchase_return_list(deleteArray);
                setTimeout(() => {
    ...
    const searchPurchase = () => {
        if(company <= 0) {
            ...
            const axios = apiUrl.get("/purchase/purchase-return-list/?company="+company+"&branch="+branch+"&from_date="+from_date+"&to_date="+to_date+"&status="+status+"&search="+search);
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.purchase_return_list} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.purchase_return_list}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.purchase_return_list}</span>
                            </div>
                        </div>
                        <div className="d-flex align-items-center d-print-none">
                            ...
                            <Link className="btn btn-success rounded-pill pd-t-6-force pd-b-5-force mg-r-3" title={lang.purchase_return} href="/purchase/purchase-return"><i className="fal fa-plus"></i></Link>
                            : ''}
                    ...
                    <div className="col-md-12 mg-b-15">
                        ...
                        <div className="table-responsive">
                            <table className="table table-striped table-bordered" ref={excelExportRef}>
                                ...
                                :
                                <tbody>
                                    {purchase_return_list.map((row, index) => {
                                    return (
                                    <tr className='' key={row.purchase_return_id}>
                                        ...
                                        <td className="tx-left">{row.purchase_return_warehouse_name}</td>
                                        ...
                                        <td className="tx-center">{row.purchase_return_status==1?lang.active:lang.inactive}</td>
                                        <td className="tx-center d-print-none">
                                            <Link className="text-info mg-r-3" href="#" onClick={()=> viewReceipt(row.purchase_return_id)} title={lang.receipt}><i className="fas fa-file-invoice-dollar wd-16 mr-2"></i></Link>
                                            <Link className="text-warning mg-r-3" href="#" onClick={()=> viewInvoice(row.purchase_return_id)} title={lang.invoice}><i className="fas fa-file-invoice wd-16 mr-2"></i></Link>
                                            {user_user_group == 1 || user_user_group == 2 || user_user_group == 3 || user_user_group == 4 || user_user_group == 5?
                                            <Link className="text-primary mg-r-3" href={`/purchase/purchase-return-edit/${row.purchase_return_id}`} title={lang.edit}><i className="fas fa-pencil wd-16 mr-2"></i></Link>
                                            :''}
                                            {/* {user_user_group == 1 ?
                                            <Link className="text-danger" href="#" title={lang.remove} onClick={() => formModalDelete(row.purchase_return_id)}><i className="fas fa-times wd-16 mr-2"></i></Link>
                                            :''} */}
                                        </td>
                                    </tr>
                                    )})}
                ...
                <div className="modal-dialog modal-md">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer border-top p-2">
                            <button type="submit" className="btn btn-sm btn-success rounded-pill text-uppercase pd-t-6-force pd-b-5-force" onClick={() => formSubmitDelete(purchase_return_id)}><i className="fal fa-check-circle"></i> {lang.yes}</button>
