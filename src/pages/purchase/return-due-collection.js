...
const ReturnDueCollection = ()=> {
    ...
    if (typeof window !== 'undefined') {
        ...
        // user_group =1 Super Admin, user_group =2 Admin, user_group =3 Manager, user_group =4 Accounts User, user_group =5 Sales & Purchase_return, user_group =6 Sales, user_group =7 Purchase_return
    ...
    const [purchase_return_due_list, setPurchase_return_due_list] = useState([]);
    ...
    const total_payable_amount                      = purchase_return_due_list.reduce((payable_amount, data) => payable_amount + parseFloat((data.payable_amount)), 0);
    const total_paid_amount                         = purchase_return_due_list.reduce((paid_amount, data) => paid_amount + parseFloat((data.paid_amount)), 0);
    const total_due_amount                          = purchase_return_due_list.reduce((due_amount, data) => due_amount + parseFloat((data.due_amount)), 0);
    ...
    const editItem = (row) => {
        ...
        const updatedArray = purchase_return_due_list.map((data) =>
            data.purchase_return_id === row.purchase_return_id ? items : data
        );

        setPurchase_return_due_list(updatedArray);
    ...
    const formSubmit = (e) => {
        ...
        const due_list = purchase_return_due_list.filter((data) => data.new_payment > 0);
        if(company <= 0) {
            ...
            const purchase_return_data = due_list.map((row) => ({
                purchase_return_id                         : row.purchase_return_id,
                purchase_return_paid_amount                : parseFloat(row.paid_amount)+parseFloat(row.new_payment),
                purchase_return_due_amount                 : parseFloat(row.payable_amount)-(parseFloat(row.paid_amount)+parseFloat(row.new_payment)),
                purchase_return_reference_number           : reference_number,
                purchase_return_payment_type               : payment_type,
                purchase_return_payment_method             : payment_method,
                purchase_return_payment_status             : parseFloat(row.payable_amount)-(parseFloat(row.paid_amount)+parseFloat(row.new_payment)) > 0?'Due':'Paid',
            }));

            const supplier_payment_return_data = due_list.map((row) => ({
                supplier_payment_return_company            : company,
                supplier_payment_return_branch             : branch,
                supplier_payment_return_date               : payment_date,
                supplier_payment_return_purchase           : row.purchase_return_purchase,
                supplier_payment_return_purchase_invoice   : row.purchase_return_purchase_invoice,
                supplier_payment_return_purchase_return    : row.purchase_return_id,
                supplier_payment_return_supplier           : supplier,
                supplier_payment_return_payable            : row.due_amount,
                supplier_payment_return_paid               : row.new_payment,
                supplier_payment_return_due                : parseFloat(row.due_amount)-parseFloat(row.new_payment),
                supplier_payment_return_type               : 'Due',
                supplier_payment_return_purchase_reference_number  : reference_number,
                supplier_payment_return_purchase_payment_type      : payment_type,
                supplier_payment_return_purchase_payment_method    : payment_method,
                supplier_payment_return_status             : payment_status
            }));

            const axios = apiUrl.post("/purchase/return-due-collection-create/",{payment_data:payment_data, purchase_return_data:purchase_return_data, supplier_payment_return_data:supplier_payment_return_data})
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
    ...
    const viewInvoice = (supplier, payment_date) => {
        window.open("/purchase/return-due-collection-invoice/?company="+company+"&branch="+branch+"&supplier="+supplier+"&payment_date="+payment_date, "Popup", "width=700, height=700");
    };

    const viewReceipt = (supplier, payment_date) => {
        window.open("/purchase/return-due-collection-receipt/?company="+company+"&branch="+branch+"&supplier="+supplier+"&payment_date="+payment_date, "Popup", "width=700, height=700");
    };

    const searchPurchaseReturnDue = () => {
        if(company <= 0) {
            setMassage('Select Company');
            setWarningModal(true);
            ...
            const axios = apiUrl.get("/purchase/purchase-return-due-list/?company="+company+"&branch="+branch+"&supplier="+supplier);
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
                    const purchase_return_data = result_data.data.map((row) => ({
                        purchase_return_id         : row.purchase_return_id,
                        purchase_return_date       : row.purchase_return_date,
                        purchase_return_purchase    : row.purchase_return_purchase,
                        purchase_return_purchase_invoice: row.purchase_return_purchase_invoice,
                        payable_amount      : row.purchase_return_payable_amount,
                        paid_amount         : row.purchase_return_paid_amount,
                        due_amount          : row.purchase_return_due_amount,
                        new_payment         : 0
                    }));
                    setPurchase_return_due_list(purchase_return_data || []);
                    setSearchButton(false);
                } else {
                    setPurchase_return_due_list([]);
                    setSearchButton(false);
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.return+' '+lang.due_collection} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.return+' '+lang.due_collection}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <Link className="breadcrumb-item" href="/purchase_return">{lang.purchase_return}</Link>
                                <span className="breadcrumb-item hidden-xs active">{lang.return+' '+lang.due_collection}</span>
                            </div>
                        </div>
                    ...
                    <div className="row justify-content-center">
                        <div className="col-md-12 col-sm-12 mb-3">
                            <div className="row mb-3 d-print-none">
                                ...
                                <div className="col-lg-2 col-md-2 col-sm-12 mt-3">
                                    <div className="form-group">
                                        <label className="form-label tx-semibold" htmlFor="payment_date">{lang.collection_date}</label>
                                ...
                                <div className="col-lg-2 col-md-2 mt-3">
                                    <div className="form-group">
                                        ...
                                        <div className="d-grid gap-2">
                                            <button type="submit" className={`btn btn-info pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchPurchaseReturnDue()}>{searchButton?lang.process: lang.search}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            {purchase_return_due_list.length > 0?
                            <div className="table-responsive">
                                <table className="table table-striped table-bordered">
                                    <thead className="tx-12 tx-uppercase">
                                        <tr>
                                            <th className="tx-center">{lang.sn}</th>
                                            <th className="tx-center">{lang.return+' '+lang.date}</th>
                                    ...
                                    :
                                    <tbody>
                                        {purchase_return_due_list.map((row, index) => {
                                        return (
...
export default ReturnDueCollection;