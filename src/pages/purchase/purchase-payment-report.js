...
const PurchasePaymentReport = ()=> {
    ...
    const payment_type_list = [
        {
            payment_type_id:1,
            payment_type_code: 'Purchase',
            payment_type_name: 'On Purchase'
        },
        {
            payment_type_id:2,
            payment_type_code: 'Due',
            payment_type_name: 'Due'
        }
    ];
    ...
    const [purchase_payment_report, setPurchase_payment_report] = useState([]);
    const total_payable_amount  = purchase_payment_report.reduce((amount, data) => amount + parseFloat((data.supplier_payment_payable)), 0);
    const total_paid_amount     = purchase_payment_report.reduce((amount, data) => amount + parseFloat((data.supplier_payment_paid)), 0);
    const total_due_amount      = purchase_payment_report.reduce((amount, data) => amount + parseFloat((data.supplier_payment_due)), 0);
    ...
    const [payment_type, setPayment_type]               = useState('all');
    ...
    const searchPurchasePaymentReport = () => {
        setSearchButton(true);
        const axios = apiUrl.get("/purchase/purchase-payment-report/?company="+company+"&branch="+branch+"&supplier="+supplier+"&payment_type="+payment_type+"&from_date="+from_date+"&to_date="+to_date+"&status="+status);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
                setPurchase_payment_report(result_data.data);
                setCompany_info(result_data.company);
                setBranch_info(result_data.branch);
                setSupplier_info(result_data.supplier);
                setSearchButton(false);
            } else {
                setPurchase_payment_report([]);
                setCompany_info('');
                setBranch_info('');
                setSupplier_info('');
                setSearchButton(false);
    ...
    return (
        ...
        <Layout>
            <HeaderTitle title={lang.purchase_payment_report} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.purchase_payment_report}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.purchase_payment_report}</span>
                            </div>
                        </div>
                ...
                <div className="row">
                    <div className="col-md-12 mg-b-15">
                        <div className="row clearfix d-print-none">
                            ...
                            <div className="col-md-3 mt-3">
                                <div className="form-group">
                                    ...
                                    <select type="text" className="form-control bd-info" id="payment_type" name="payment_type" value={payment_type} onChange={(e) => setPayment_type(e.target.value)}>
                                        <option value="all">All</option>
                                        {payment_type_list.map(payment_type_row => (
                                        <option key={payment_type_row.payment_type_code} value={payment_type_row.payment_type_code}>{payment_type_row.payment_type_name}</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="col-md-3 mt-3">
                                <div className="form-group">
                                    ...
                                    <div className="d-grid gap-2">
                                        <button type="submit" className={`btn btn-success pd-t-6-force pd-b-5-force mg-r-3 tx-uppercase ${searchButton?'disabled': ''}`} onClick={() => searchPurchasePaymentReport()}>{searchButton?lang.process: lang.search}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.purchase_payment_report}</span>
                                            </th>
                        ...
                        {searchButton?
                            ...
                            :<>
                            {purchase_payment_report.length > 0 ?
                            <>
                            <table className="table table-striped table-bordered" width="100%">
                                ...
                                :
                                <tbody>
                                    {purchase_payment_report.map((row, index) => {
                                    return (
                                    <tr className='' key={row.supplier_payment_id}>
                                        <td className="tx-center">{(index+1).toString().padStart(2, '0')}</td>
                                        <td className="tx-center">{new Date(row.supplier_payment_date).toLocaleString('en-in', {day: '2-digit', month:'2-digit', year: 'numeric'})}</td>
                                        <td className="tx-center">{row.supplier_payment_purchase_invoice}</td>
                                        <td className="tx-left">{row.supplier_payment_supplier_name}</td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.supplier_payment_payable}/></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.supplier_payment_paid}/></td>
                                        <td className="tx-right"><AccountsNumberFormat amount={row.supplier_payment_due}/></td>
                                        <td className="tx-center">{row.supplier_payment_type}</td>
                                    </tr>
                                    )})}
                                    <tr className="">
