...
const BalanceSheet = ()=> {
    ...
    const [balance_sheet_list, setBalance_sheet_list]   = useState([]);
    ...
    const searchBalanceSheet = ()=> {
        ...
        if(company <= 0) {
            ...
            const axios = apiUrl.get("/accounts/balance-sheet/?company="+company+"&branch="+branch+"&closing_month="+closing_month);
            axios.then((response) => {
                const result_data = response.data;
                if(result_data.status == 1){
                    ...
                    setBalance_sheet_list(result_data.data.coa_data);
                    setSearchButton(false);
                } else {
                    setCompany_info('')
                    setBranch_info('')
    ...
    const viewNote = (data) => {
        ...
        const axios = apiUrl.get("accounts/get-balance-sheet-note/?note="+data.note+"&company="+data.company+"&branch="+data.branch+"&closing_month="+data.closing_month+"&coa="+data.coa);
        axios.then((response) => {
            const result_data = response.data;
            if(result_data.status == 1){
    ...
    return (
        <Layout>
            <HeaderTitle title={lang.balance_sheet} keywords='' description=''/>
                    ...
                    <div className="d-flex justify-content-between">
                        <div className="clearfix">
                            <div className="pd-t-5 pd-b-5 d-print-none">
                                <h2 className="pd-0 mg-0 tx-14 tx-dark tx-bold tx-uppercase">{lang.balance_sheet}</h2>
                            </div>
                            <div className="breadcrumb pd-0 mg-0 d-print-none">
                                ...
                                <span className="breadcrumb-item hidden-xs active">{lang.balance_sheet}</span>
                            </div>
                        </div>
                    ...
                    <div className="col-md-12 mg-b-15">
                                    ...
                                    <tbody>
                                        <tr className="">
                                            ...
                                            <th className="tx-center tx-uppercase" width="40%" valign="top">
                                                <span className='tx-uppercase tx-16 text-decoration-underline'>{lang.balance_sheet}</span>
                                            </th>
                        ...
                        <div className="table-responsive">
                            ...
                            <table className="table-striped table-hover table-centered align-middle table-nowrap" width="100%">
                                ...
                                <tbody>
                                    {(() => {
                                        let balance_sheet_note_cg   = 0;
                                        let balance_sheet_note_gl   = 0;
                                        let balance_sheet_note_gl_2 = 0;
                                        {
                                            return (<>
                                            {balance_sheet_list.map((at_row) => (
                                                <>
                                                    <tr className='' key={at_row.chart_of_accounts_id}>
                                                        <th className="tx-left tx-uppercase" colSpan={12}>{at_row.chart_of_accounts_code} - {at_row.chart_of_accounts_name}</th>
                                                    </tr>
                                                    {at_row.accounts_category.map((cat_row) => (
                                                    <>
                                                        <tr className='' key={cat_row.chart_of_accounts_id}>
                                                            <th className="tx-left">{cat_row.chart_of_accounts_code} - {cat_row.chart_of_accounts_name}</th>
                                                        ...
                                                        <>
                                                            {/* <tr className='' key={cg_row.chart_of_accounts_id}>
                                                                <td className="tx-left">{cg_row.chart_of_accounts_code} - {cg_row.chart_of_accounts_name}</td>
                                                                <td className="tx-center">{(++balance_sheet_note_cg).toString().padStart(2, '0')}</td>
                                                            ...
                                                            <>
                                                                <tr className='' key={gl_row.chart_of_accounts_id}>
                                                                    <td className="tx-left">{gl_row.chart_of_accounts_code} - {gl_row.chart_of_accounts_name}</td>
                                                                    <td className="tx-center"><a href="#" onClick={() => viewNote({note: ++balance_sheet_note_gl_2, company: company, branch: branch, closing_month: closing_month, coa: gl_row.chart_of_accounts_id})}>{(++balance_sheet_note_gl).toString().padStart(2, '0')}</a></td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th className="tx-center" width="1%"></th>
                                                        <th className="tx-right bd-top bd-bottom-double"><AccountsNumberFormat amount={at_row.balance.current_month_balance} /></th>
                                                        <th className="tx-center" width="1%"></th>
                                                        <th className="tx-right bd-top bd-bottom-double"><AccountsNumberFormat amount={at_row.balance.last_month_balance} /></th>
                                                        <th className="tx-center" width="1%"></th>
                                                        <th className="tx-right bd-top bd-bottom-double"><AccountsNumberFormat amount={at_row.balance.current_year_balance} /></th>
                                                        <th className="tx-center" width="1%"></th>
                                                        <th className="tx-right bd-top bd-bottom-double"><AccountsNumberFormat amount={at_row.balance.last_year_balance} /></th>
                                                    </tr>
                                                </>
                                                ))}
                                            </>);
